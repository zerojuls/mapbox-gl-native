...
void OfflineDatabase::handleError(const std::exception& ex, const char* action) {
    if (auto sqliteEx = dynamic_cast<const mapbox::sqlite::Exception*>(&ex)) {
        if (sqliteEx->code == mapbox::sqlite::ResultCode::NotADB ||
            sqliteEx->code == mapbox::sqlite::ResultCode::Corrupt) {
            // Corrupted; delete database so that we have a clean slate for the next operation.
            Log::Error(Event::Database, static_cast<int>(sqliteEx->code), "Can't %s: %s", action, sqliteEx->what());
            try {
                removeExisting();
            } catch (const util::IOException& ioEx) {
                Log::Error(Event::Database, ioEx.code, "Can't %s: %s", action, ioEx.what());
            }
        } else {
            // We treat the error as temporary, and pretend we have an inaccessible DB.
            Log::Warning(Event::Database, static_cast<int>(sqliteEx->code), "Can't %s: %s", action, sqliteEx->what());
        }
    } else if (auto ioEx = dynamic_cast<const util::IOException*>(&ex)) {
        // We failed to delete the database file.
        Log::Error(Event::Database, ioEx->code, "Can't %s: %s", action, ioEx->what());
    } else {
        Log::Error(Event::Database, -1, "Can't %s: unknown error", action);
    }
}
