...
public class RuntimeStyleActivity extends AppCompatActivity {
  ...
  private void addParksLayer() {
    ...
    layer.setFilter(all(eq(literal("type"), literal("park")), eq(get("stroke-width"), literal(2))));
  ...
  private void styleLineFilterLayer() {
    ...
    handler.postDelayed(() -> {
      ...
      if (counties != null) {
        counties.setFilter(eq(string(get("NAME10")), literal("Washington")));
  ...
  private void styleNumericFillLayer() {
    ...
    handler.postDelayed(() -> {
      ...
      if (regions != null) {
        regions.setFilter(all(gte(get("HRRNUM"), literal(200)), lt(get("HRRNUM"), literal(300))));
